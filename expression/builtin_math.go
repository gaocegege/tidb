...
func (b *builtinRoundSig) eval(row []types.Datum) (d types.Datum, err error) {
	...
	if args[0].Kind() == types.KindMysqlDecimal {
		...
		err = args[0].GetMysqlDecimal().Round(&dec, frac, types.NormalRound)
		if err != nil {
			return d, errors.Trace(err)
		}
...
func (b *builtinTruncateSig) eval(row []types.Datum) (d types.Datum, err error) {
	...
	if args[0].Kind() == types.KindMysqlDecimal {
		var dec types.MyDecimal
		err = args[0].GetMysqlDecimal().Round(&dec, frac, types.NeverRound)
		if err != nil {
			return d, errors.Trace(err)
		}
		d.SetMysqlDecimal(&dec)
	...
	// Get the truncate.
	shift := math.Pow10(frac)
	val := math.Trunc(x*shift) / shift
